...
(* $Id: avl_graphviz.ml,v 1.2 2003/06/26 13:32:45 simonet Exp $ *)
...
module CommonAttributes = struct
  ...
  let fprint_graph ppf = function
    ...
    | `Label s -> fprintf ppf "label=%a" fprint_string s
  ...
  let fprint_edge ppf = function
    ...
    | `Labelfontname s -> fprintf ppf "labelfontname=\"%s\"" (String.escaped s)
...
module type ENGINE = sig
  ...
  module Attributes : sig
    ...
    val fprint_node: formatter -> node -> unit
...
module MakeEngine (E: ENGINE) = struct

  let command = ref E.name
  let set_command cmd =
    command := cmd
  ...
  module type INPUT = sig
    ...
    val graph_attributes: graph -> E.Attributes.graph list
    val default_node_attributes: graph -> E.Attributes.node list
    val default_edge_attributes: graph -> E.Attributes.edge list

    val node_name: node -> string
    val node_attributes: node -> E.Attributes.node list

    val edge_head: edge -> node
    val edge_tail: edge -> node
    val edge_attributes: edge -> E.Attributes.edge list
  ...
  module Make (X: INPUT) = struct
    ...
    *)
    let fprint_graph_attributes ppf list =
	 ...
	 fprintf ppf "%a;@ " E.Attributes.fprint_graph att
       ) list
    ...
    *)
    ...
    let fprint_node_attributes ppf list = 
      fprint_attributes E.Attributes.fprint_node ppf list
	 
    (** [fprint_graph_attributes ppf list] pretty prints a list of 
        edge attributes using the format of [fprint_attributes].
     *)
    let fprint_edge_attributes ppf list =
      fprint_attributes E.Attributes.fprint_edge ppf list
	...
	let default_node_attributes = X.default_node_attributes graph in
	...
	X.iter_nodes (function node ->
	  fprintf ppf "%s%a;@ " 
	    (X.node_name node)
	    fprint_node_attributes (X.node_attributes node);
        ) graph
	...
	X.iter_edges (function edge ->
	  fprintf ppf "%s %s %s%a;@ "
	    (X.node_name (X.edge_head edge))
	    E.edge_arrow
	    (X.node_name (X.edge_tail edge))
     ...
     *)
    let run_graph output_mode f graph =
	...
	| `Ismap -> "ismap"
      ...
      let ic, oc = 
	Unix.open_process (Printf.sprintf "%s -T%s" !command format)
      in
	...
	Unix.WEXITED 0 -> result
      | Unix.WEXITED n -> 
	  raise (Error (Printf.sprintf "process exits with return code %i" n))
      | Unix.WSIGNALED n ->
	  raise (Error (Printf.sprintf "process killed by signal %i" n))
      | Unix.WSTOPPED n ->
	  raise (Error (Printf.sprintf "process stopped by signal %i" n))
...
module DotAttributes = struct
  ...
  let fprint_graph ppf = function
    ...
    | `Url s -> fprintf ppf "URL=\"%s\"" (String.escaped s)

  let fprint_node ppf = function
      #CommonAttributes.node as att -> CommonAttributes.fprint_node ppf att
    | `Comment s -> fprintf ppf "comment=%a" fprint_string s
