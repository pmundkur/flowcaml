...
(* $Id: location.ml,v 1.2 2003/06/26 13:32:49 simonet Exp $ *)
(* Location: locations in source code *)
...
let union loc1 loc2 =
  { loc_start = min loc1.loc_start loc2.loc_start;
    loc_end = max loc1.loc_end loc2.loc_end;
...
let rhs_locs n m =
  { loc_start = Parsing.rhs_start n; loc_end = Parsing.rhs_end m;
    loc_ghost = false }
...
let print_warning loc ppf w =
  () (* TEMPORARY
  if Warnings.is_active w false then begin
    let printw ppf w =
